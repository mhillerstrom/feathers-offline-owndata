...
const debug = makeDebug('ownnet-mutator');
...
class Service extends AdapterService {
  constructor (options = {}) {
    ...
    if (!this._engine.useUuid) {
      throw new Error('Replicator must be configured for uuid for optimistic updates. (ownnet)');
    }

    if (!this._engine.useUpdatedAt) {
      throw new Error('Replicator must be configured for updatedAt for optimistic updates. (ownnet)');
    }

    this._mutateStore = this._engine._mutateStore.bind(this._engine);
    this._addQueuedNetEvent = this._engine._addQueuedNetEvent.bind(this._engine);
    this._removeQueuedNetEvent = this._engine._removeQueuedNetEvent.bind(this._engine);
  ...
  async _create (data, params = {}) {
    ...
    if (index > -1) {
      throw new errors.BadRequest('Optimistic create requires unique uuid. (ownnet)');
    }

    // optimistic mutation
    const newData = this._mutateStore('created', data, 1);
    this._addQueuedNetEvent('create', newData, shallowClone(newData), params);
    ...
    await this.remoteCreate(shallowClone(newData), params)
      .then(([err, res]) => {
        ...
        if (res) {
          this._removeQueuedNetEvent('create', newData, res.updatedAt);
        }
      })
  ...
  async _update (id, data, params = {}) {
    ...
    this._addQueuedNetEvent('update', newData, getId(newData), shallowClone(newData), params);
    ...
    await this.remoteUpdate(getId(newData), shallowClone(newData), params)
      .then(([err, res]) => {
        ...
        if (res) {
          this._removeQueuedNetEvent('update', shallowClone(newData), res.updatedAt);
        }
      });
    return Promise.resolve(newData)
      .then(select(params, ...this._alwaysSelect));
  ...
  async _patch (id, data, params = {}) {
    ...
    this._addQueuedNetEvent('patch', newData, getId(newData), shallowClone(newData), params);
    ...
    await this.remotePatch(getId(newData), shallowClone(newData), params)
      .then(([err, res]) => {
        ...
        if (res) {
          this._removeQueuedNetEvent('patch', newData, res.updatedAt);
        }
      });
  ...
  async _remove (id, params = {}) {
    ...
    this._addQueuedNetEvent('remove', newData, getId(newData), params);

    // Start actual mutation on remote service
    await this.remoteRemove(getId(newData), params)
      .then(([err, res]) => {
        if (err) {
          if (err.timeout) {
        ...
        if (res) {
          this._removeQueuedNetEvent('remove', newData, null);
        }
      });
  ...
  _checkConnected () {
    if (!this._replicator.connected) {
      throw new errors.BadRequest('Replicator not connected to remote. (ownnet)');
    }
  }
}
...
function checkUuidExists (record) {
  if (!('uuid' in record)) {
    throw new errors.BadRequest('Optimistic mutation requires uuid. (ownnet)');
  }
}
